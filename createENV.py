...
class elb_class(env_class):
	...
	def get_conn(self):
		...
		try:
			elbc	=	boto.ec2.elb.connect_to_region(region,aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
		except Exception:
...
class ec2_class(env_class):
	...
	def get_instances(self):
		...
		try:
			...
			return False
		LOG.info ("Following running instances %s , stopped instances %s were found under ELB %s",runnig_elb_hosts,stopped_elb_hosts,elb_name) 
...
if __name__ == "__main__":
	...
	Create help
	""" 
	parser = argparse.ArgumentParser(description='Build new envinroment including one Elastic Load Balancer (ELB) with num of EC2 instances (2 by default)', formatter_class=RawTextHelpFormatter)
	...
	parser.add_argument('--num_of_instancess',type=int,help='number of instances to build and assign to ELB (2 by default)')
	...
	if action == 'create':
		print "Creating a environment ",name,". Please wait"
		env_o.create()
		elb_o		=	elb_class(name, elb_name, instance_id, num_of_instances)
		...
		print "Deleting environment ",name
		env_o.destroy()
